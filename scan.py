    ...
    for dist in os.getenv('DISTS', 'trusty').split(','):
        for arch in os.getenv('ARCHS', 'amd64,i386').split(','):
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            sp.check_call(['/practice.sh', path], shell=True)
            # cmd = 'dpkg-scanpackages -m . | gzip -9c > {0}/Packages.gz'.format(
            #    path)
            # sp.check_call(cmd, shell=True, close_fds=True)
            # release_path = '/data/dists/{}/main/binary-{}/Release'.format(
            #    dist, arch)

            # sp.check_output(
            #    "cat conf/distributions > {0}".format(release_path), shell=True)
            # sp.call('cat conf/distributions > {0}'.format(release_path))
    info('Scanning...done')
